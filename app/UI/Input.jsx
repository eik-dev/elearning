...
export default function Input({type, value, setValue, placeholder, disabled, name, required }){
    ...
    return (
        <div id={name.replace(/\s+/g, '').toLowerCase()} className={`border-2 border-primary rounded-md focus-within:border-secondary focus-within:text-Grey py-2 pr-4 relative h-fit w-full ${error?'text-warning border-warning':'text-gray-500'} bg-primary-base`}>
            <span className="text-xs absolute -top-2 left-2 px-2 font-semibold bg-[#EAEFD8]">{name} {required ? '*' : ''}</span>
            {
                type=='textarea'?
